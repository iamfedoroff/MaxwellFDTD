function test_intensity_losses_2D(fname, ki)
    ...
    Sa = [mask[iz] ? 0.0 : Sa[ix,iz] for ix=1:Nx, iz=1:Nz]
    Sa .= Sa ./ maximum(Sa)

    I = [mask[iz] ? 0.0 : exp(-2 * ki * z[iz]) for ix=1:Nx, iz=1:Nz]
...
# ******************************************************************************************
grid = Grid2D(
    xmin=-25e-6, xmax=25e-6, Nx=61,
    zmin=-15e-6, zmax=25e-6, Nz=601,
)
...
solve!(model; fname)
@test test_intensity_losses_2D(fname, ki)
