function test_intensity_losses_3D(fname, ki)
    ...
    Sa = [mask[iz] ? 0.0 : Sa[ix,iy,iz] for ix=1:Nx, iy=1:Ny, iz=1:Nz]
    Sa .= Sa ./ maximum(Sa)

    I = [mask[iz] ? 0.0 : exp(-2 * ki * z[iz]) for ix=1:Nx, iy=1:Ny, iz=1:Nz]
...
# ******************************************************************************************
grid = Grid(
    xmin=-25e-6, xmax=25e-6, Nx=11,
    ymin=-25e-6, ymax=25e-6, Ny=11,
