...
# ******************************************************************************************
grid = Grid2D(
    xmin=-42e-6, xmax=42e-6, Nx=1681,
    zmin=-4e-6, zmax=6e-6, Nz=201,
)
...
function source_geometry(x, z)
    return abs(z - lam0/2) < grid.dz && abs(x - 0) < grid.dx/2
end
...
inspect("out.hdf", :Ex; colorrange=(-1e-3,1e-3), aspect=grid.Lx/grid.Lz)
