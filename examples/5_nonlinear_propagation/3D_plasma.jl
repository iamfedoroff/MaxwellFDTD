# ******************************************************************************************
# WARNING!!! The output file is 24 Gb.
# ******************************************************************************************
using MaxwellFDTD
using MaxwellPlots
...
# ******************************************************************************************
grid = Grid(
    xmin=-32e-6, xmax=32e-6, Nx=156,
    ymin=-32e-6, ymax=32e-6, Ny=156,
    # zmin=-2e-6, zmax=102e-6, Nz=1041,
    zmin=-2e-6, zmax=72e-6, Nz=741,
)
...
function source_waveform(x, y, z, t, p)
    w0, tau0, dt0, a0, E0 = p
    A = E0 * exp(-(x^2 + y^2) / a0^2)    # Gaussian beam
    return A * exp(-0.5 * (t - dt0)^2 / tau0^2) * cos(w0 * (t - dt0))
end
...
# Model & Solve
# ******************************************************************************************
# model = Model(grid, source, tmax=700e-15, pml=2e-6, material=SiO2)
model = Model(grid, source, tmax=550e-15, pml=2e-6, material=SiO2)
...
# ******************************************************************************************
inspect("out.hdf", :Ex)
# inspect3D_xsec("out.hdf", :Ex)
# plot3D("out.hdf", :Sa; colorrange=(0,0.5))

# inspect("out.hdf", :rho; colorrange=(0,0.5))
# inspect3D_xsec("out.hdf", :rho; colorrange=(0,0.5))
# plot3D("out.hdf", :rho_end; colorrange=(0,0.5))
# plot3D_xsec("out.hdf", :rho_end)
# plot3D_xsec("out.hdf", :JE)
