...
function material_geometry(x, y, z)
    ...
    # return z >= 0 && z <= 10e-6   # material slab from z=0 to z=10um
    return sqrt(x^2 + y^2 + z^2) <= 10e-6   # material is inside the sphere of radius 10um
    ...
    # Slab with two pin-holes:
    # return (z >= 0 && z <= 5e-6) &&
    #        ! (sqrt(x^2 + (y-7e-6)^2) <= 4e-6 || sqrt(x^2 + (y+7e-6)^2) <= 4e-6)
end
